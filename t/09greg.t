...
for (
    ...
    is(join('/',DateTime::jd2greg($_->[0])), join('/',@{$_}[1..3]),
       $_->[0]."   \t=> ".join'/',@{$_}[1..3]);
    is(DateTime::greg2jd(@{$_}[1..3]), $_->[0],
       join('/',@{$_}[1..3])."   \t=> ".$_->[0]);
}
...
while ( $y <= 4800 ) {
    ...
    $dno += $mlen - 1;
    $dno2 = DateTime::greg2jd( $y, $m, $mlen );
    if ( $dno != $dno2 ) {
        is( $dno2, $dno,
